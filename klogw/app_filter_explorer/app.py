...
def sanitize_json(obj):
    ...
    if isinstance(obj, complex):
        if abs(obj.imag) < 1e-14:
            ...
            raise ValueError(f"Truly complex: {obj}")
...
def design_filter(family, ftype, order, domain, c1, c2):
    ...
    try:
        if family == "Butterworth":
            ...
            z, p, k = signal.cheby1(
                order, 1, Wn, btype=ftype, analog=analog, output="zpk"
            )
        elif family == "Chebyshev II":
            z, p, k = signal.cheby2(
                order, 40, Wn, btype=ftype, analog=analog, output="zpk"
            )
        elif family == "Elliptic":
            z, p, k = signal.ellip(
                order, 1, 40, Wn, btype=ftype, analog=analog, output="zpk"
            )
        elif family == "Bessel":
...
def update_all(
    fam, ftype, order, domain, c1, c2, addp, addz, clear_btn, relayoutData, store_data
):
    ctx = callback_context
    trig_id = ctx.triggered[0]["prop_id"].split(".")[0] if ctx.triggered else None

    print(f"Triggered by: {trig_id}")
    ...
    # If user dragged shapes => unify lines & zeros
    if relayoutData and isinstance(relayoutData, dict) and "shapes" in relayoutData:
        shapes_array = relayoutData["shapes"]
        # We'll gather final centers for each zero or pole
        zero_map = {}
        pole_lines = {}

        for shape_dict in shapes_array:
        ...
        # Now unify zeros
        for zidx, (cx, cy) in zero_map.items():
            old_zeros[zidx] = complex(cx, cy)

        # unify poles: if line0 or line1 missing, we take the old location
        # then average
        for pidx in range(len(old_poles)):
            if pidx not in pole_lines:
                # user never dragged either line => keep old
                continue
            coords = pole_lines[pidx]
            if len(coords) == 1:
                # user only dragged line0 or line1, the other line wasn't in shapes
                # => reconstruct center from old store
    ...
    fig_pz = {
        ...
        "layout": {
            ...
            # crucial => preserve user shape changes
            "uirevision": "pz-uirev",
    ...
    # poles => lines "pole-{pidx}-line0"/"pole-{pidx}-line1"
