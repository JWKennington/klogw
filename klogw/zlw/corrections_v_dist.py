...
# Template‐intrinsic ranges (if VARY_TEMPLATE_INTRINSICS=True)
...
# If VARY_PSD=False, use this fixed‐perturbation
PSD2_FIXED_KIND   = "gaussian"
PSD2_FIXED_KWARGS = {"center":150.0, "width":50.0, "amplitude_ratio":0.3}

# If VARY_PSD=True, randomly pick from this list each injection:
PERTURBATION_LIST = [
...
for i in range(n_injections):
    ...
    # (Uses the real+imag interpolation so phase is preserved)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    M = len(freqs)  # = N//2 + 1
    H1_full = np.zeros(N, dtype=complex)
    H1_full[:M] = H0_template * W1
    H1_full[M:] = np.conj((H0_template * W1)[-2:0:-1])
    h1 = np.fft.ifft(H1_full)  # complex time series

    # ─────────────────────────────────────────────────────────────────────────
    # (F.7) BUILD FULLY COMPLEX “WHITENED INJECTION” x2(t)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    if ADD_PSD_SHAPED_NOISE:
        noise_t_raw = make_whitened_noise(PSD2, dt)  # time‐domain RMS ≈ “large”
        ...
        snr0 = np.max(np.abs(z0))  # might be small if waveform is weak

        # (F.7.3) boost signal so that snr0 → SNR_target if needed
        SNR_target = 10.0
        if snr0 > 0:
    ...
    phi_hat  = np.angle(z_t_complex[idx_peak])  # ☆ correct phase extraction
    ...
    if i < 3:
        ...
        # Print true vs. estimated times and phases next to eachother
        ...
        plt.plot(times, x2_clean.real, label="Clean injection", color='blue')
        plt.plot(times, x2_noisy.real, label="Noisy injection", color='orange', alpha=0.3)
