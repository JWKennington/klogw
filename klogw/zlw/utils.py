...
def compute_minimum_phase_phi(psd, freqs, N):
    ...
    # 2) Positive‐half log‐amplitude
    logA_pos = -0.5 * np.log(psd2)

    # 3) Build full length‐N real vector via Hermitian mirroring
    logA_full = np.zeros(N, dtype=np.float64)
    logA_full[:M] = logA_pos
    logA_full[M:] = logA_pos[-2:0:-1]

    # 4) Compute cepstrum = IFFT(logA_full).real
    cepstrum = np.fft.ifft(logA_full).real

    # 5) Build minimum‐phase cepstrum
    ...
    # 6) FFT back to get full complex log spectrum
    logMin_full = np.fft.fft(minp_cep)

    # 7) Extract phase on positive half
...
def generate_fd_waveform(f_min, f_max, df, m1, m2, distance_mpc, approximant=lalsim.TaylorF2):
    ...
    # Spins = 0, orbit params = 0 (circular, non-precessing)
...
def interpolate_H0(freqs_full, freqs_fd, H0_fd):
    ...
    Returns:
      – H0_full: complex array of length freqs_full.shape, where
           H0_full[0] = 0.0 (DC)
           H0_full[1:] = np.interp(freqs_full[1:], freqs_fd, H0_fd[1:], left=0, right=0)
    """
    H0_full = np.zeros_like(freqs_full, dtype=np.complex128)
    # Skip DC (index 0)
    H0_full[1:] = np.interp(freqs_full[1:], freqs_fd, H0_fd[1:], left=0.0, right=0.0)
    H0_full[0] = 0.0
    return H0_full
...
def compute_corrections(H0, PSD2, phi_mp, freqs):
    ...
    Returns:
      – (dt1, dphi1, dt2, dphi2)
    """
    # 1) Build spectral weight W(f) = |H0|^2 / PSD2
    Wf = (np.abs(H0) ** 2) / PSD2

    # 2) Restrict to positive frequencies excluding DC (index 0)
    fpos   = freqs[1:]
    Wfpos  = Wf[1:]
    Phi_pos = phi_mp[1:]
...
def match_filter_time_series(h1_t, x2_t, dt):
    ...
    """
    N = len(h1_t)
    # 1) FFT of template (complex array)
    H1_full = np.fft.fft(h1_t, n=N)

    # 2) FFT of data (complex array)
    X2_full = np.fft.fft(x2_t, n=N)

    # 3) Multiply conj(H1)*X2 (length‐N complex array)
    corr_full = np.conj(H1_full) * X2_full

    # 4) Inverse FFT → length‐N complex correlation
    z_unnorm = np.fft.ifft(corr_full, n=N)

    # 5) fftshift so that lag=0 → index N//2
    z_t = np.fft.fftshift(z_unnorm)

    # 6) Build lag axis [−T/2 … +T/2−dt]
    idx = np.arange(N)
    tc = (idx - (N // 2)) * dt
...
def make_injection_time_series(H0, W2, freqs, N, dt, t_true, phi_true):
    ...
    Steps:
      1) Construct the FD injection: H_sig(f) = H0(f)*exp(i phi_true)*exp(-2π i f t_true).
      2) Whiten in FD:          X2(f)   = H_sig(f)*W2(f).
      3) IFFT → x2(t)   (use np.fft.ifft, not irfft).
      4) “Unwrap”   :   roll so that sample idx_shift → 0.  idx_shift = round(t_true/dt).
      5) “Center”   :   roll so that index 0 → N//2 (peak at zero‐lag midpoint).
      6) Return that length‐N complex array.
    """
    # 1) FD injection phase shifts:
    phase_shift = np.exp(-2j * np.pi * freqs * t_true)
    Hsig_f = H0 * np.exp(1j * phi_true) * phase_shift

    # 2) Whiten by PSD₂'s minimum‐phase:  X2(f) is complex
    X2_f = Hsig_f * W2

    # 3) Inverse FFT → complex time series of length N
    x2_t = np.fft.irfft(X2_f, n=N)
...
def wrap_time_residuals(t_hat_arr, t_true_arr, duration):
    ...
    We do:  Δ = t_hat - t_true, then residual = (Δ + 1.5*duration) % duration - 0.5*duration.
    """
...
def plot_residuals_scatter(
    ...
    Inputs:
      – t_true_arr, t_hat_raw_arr, t_hat_corr1_arr, t_hat_corr12_arr:  1D arrays
      – phi_true_arr, phi_hat_raw_arr, phi_hat_corr1_arr, phi_hat_corr12_arr: 1D arrays
      – duration: total time duration (for drawing the diagonal “ideal” line)
    """
    fig = plt.figure(figsize=(12, 5))
    ...
    ax1.scatter(
        t_true_arr, t_hat_corr12_arr, color="magenta", alpha=0.5, label="1+2nd-order"
    )
    ...
    ax2.scatter(
        phi_true_arr, phi_hat_corr1_arr, color="green", alpha=0.5, label="1st-order"
    )
    ax2.scatter(
        phi_true_arr, phi_hat_corr12_arr, color="purple", alpha=0.5, label="1+2nd-order"
    )
